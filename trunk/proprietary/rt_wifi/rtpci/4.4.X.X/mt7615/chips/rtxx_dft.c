...
static VOID RxSensitivityTuning(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("turn off R17 tuning, restore to 0x%02x\n", R66));
}




static VOID ChipBBPAdjust(RTMP_ADAPTER *pAd, UCHAR Channel)
{
	UCHAR bbp_val;
	UINT8 rf_bw, ext_ch;
...
#ifdef DOT11_N_SUPPORT
	if (rf_bw == BW_40)
		bbp_val = (Channel > 14) ? 0x48 : 0x38;
	else
#endif /* DOT11_N_SUPPORT */
		bbp_val = (Channel > 14) ? 0x40 : 0x38;
...
static VOID Default_ChipSwitchChannel(
	...
	IN MT_SWITCH_CHANNEL_CFG SwChCfg) 
{
	UCHAR Channel = SwChCfg.CentralChannel;
	BOOLEAN bScan = SwChCfg.bScan;

	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): dummy channel switch function!\n", __FUNCTION__));
	...
	IN EEPROM_ANTENNA_STRUC	*pAntenna)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("E2PROM error, hard code as 0x%04x\n", pAntenna->word));	
}
